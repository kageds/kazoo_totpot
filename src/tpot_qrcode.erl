...
-include("tpot_qrcode.hrl").
-include("tpot_qrcode_params.hrl").
...
encode(Bin, ECC) when is_binary(Bin) ->
	...
	Matrix = tpot_qrcode_matrix:embed_data(Params, Codewords),
	MaskedMatrices = tpot_qrcode_mask:generate(Params, Matrix),
	Candidates = [tpot_qrcode_matrix:overlay_static(Params, M) || M <- MaskedMatrices],
	{MaskType, SelectedMatrix} = tpot_qrcode_mask:select(Candidates),
	...
	QRCode = tpot_qrcode_matrix:finalize(Dim, FMT, VSN, ?QUIET_ZONE, SelectedMatrix),
...
choose_qr_params(Bin, ECLevel) ->
	...
	Dim = tpot_qrcode_matrix:dimension(Version),
...
generate_ecc0(Bin, Count, TotalLength, BlockLength, Acc) when byte_size(Bin) >= BlockLength, Count > 0 ->
	...
	EC = tpot_qrcode_reedsolomon:encode(Block, TotalLength - BlockLength),
...
alignment_patterns(Version) ->
	D = tpot_qrcode_matrix:dimension(Version),
...
version_info_bits(#qr_params{version = Version}) when Version =< 40 ->
	BCH = tpot_qrcode_reedsolomon:bch_code(Version, ?VERSION_INFO_POLY),
	<<Version:6, BCH:12>>.

format_info_bits(#qr_params{ec_level = ECLevel, mask = MaskType}) ->
	Info = (ecc(ECLevel) bsl 3) bor MaskType,
	BCH = tpot_qrcode_reedsolomon:bch_code(Info, ?FORMAT_INFO_POLY),
