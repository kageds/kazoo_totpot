...
-spec dimension(integer()) -> integer().
dimension(Version) 
		when Version > 0 
		andalso Version < 41 ->
	17 + (Version * 4).
...
-spec embed_data(#qr_params{}, binary()) -> binary().
...
-spec overlay_static(#qr_params{}, binary()) -> binary().
...
-spec finalize(binary(), binary(), binary(), binary(), binary()) -> binary().
...
alignment_bits([H|T], Acc) ->
	Bits0 = tpot_bits:duplicate(<<31:5>>, H),
	Bits1 = tpot_bits:duplicate(<<17:5>>, H),
	Bits2 = tpot_bits:duplicate(<<21:5>>, H),
	Bits = tpot_bits:append([Bits0, Bits1, Bits2, Bits1, Bits0]),
...
%%
format_bits(Bin) ->
	<<A:7, C:1, E:7>> = tpot_bits:reverse(Bin),
	<<B:8, D:7>> = Bin,
	<<A:7, B:8, C:1, D:7, 1:1, E:7>>.

%%
version_bits(Bin) ->
	VTop = tpot_bits:reverse(Bin),
...
version_bits([], RowA, RowB, RowC) ->
	tpot_bits:append([RowA, RowB, RowC]).
