...
%%
-spec add(#gf256{}, integer(), integer()) -> list().
...
-spec subtract(#gf256{}, integer(), integer()) -> list().
subtract(F = #gf256{}, A, B) ->
	add(F, A, B).

%%
-spec multiply(#gf256{}, integer(), integer()) -> list().
multiply(#gf256{}, 0, _) ->
	0;
multiply(#gf256{}, _, 0) ->
	0;
...
%%
-spec exponent(#gf256{}, integer()) -> list().
exponent(#gf256{exponent = E}, X) ->
	lists:nth(X + 1, E).

%%
-spec log(#gf256{}, integer()) -> list().
log(#gf256{log = L}, X) ->
	lists:nth(X + 1, L).

%%
-spec inverse(#gf256{}, integer()) -> list().
inverse(F = #gf256{}, X) ->
	exponent(F, ?RANGE - log(F, X)).

%%
-spec value(#gf256{}, list(), integer()) -> list().
value(#gf256{}, Poly, 0) ->
	lists:last(Poly);
value(F = #gf256{}, Poly, 1) ->
	lists:foldl(fun(X, Sum) -> tpot_gf256:add(F, X, Sum) end, 0, Poly);
value(F = #gf256{}, [H|T], X) ->
	value(F, T, X, H).
%
value(F, [H|T], X, Acc) ->
...
%
-spec monomial(#gf256{}, integer(), list()) -> list().
...
%%
-spec monomial_product(#gf256{}, list(), list(), list()) -> list().
monomial_product(F, Poly, Coeff, Degree) ->
	monomial_product(F, Poly, Coeff, Degree, []).
%
monomial_product(F, [H|T], C, D, Acc) ->
	P = tpot_gf256:multiply(F, H, C),
...
%%
-spec polynomial_product(list(), list(), list()) -> list().
...
%%
-spec divide(#gf256{}, list(), list()) -> list().
